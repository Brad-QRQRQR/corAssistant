...
mse_loss = nn.MSELoss()
alphabet_character_file = open(config['alpha_path'], encoding="utf-8")                # 開啟字典檔
...
def saver():
    ...
    f = open(os.path.join('./history', config['exp_name'], str(localtime)),'w+', encoding="utf-8")
    f.close()
